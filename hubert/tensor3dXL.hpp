...
Tensor3dXL<T>::Tensor3dXL(Tensor3dXL<T> *A) //Takes 2d matrixes from the pointer and creates new Tensors based on them
{
	...
	{
		static TensorXL<float> layer(get(*A, d)); //TODO: almost certainly wrong. Dont care right now
		matrix[d] = &layer;
	}
	null = false;
}
...
Tensor3dXL<T>::Tensor3dXL(int dep, int row, int col, T init)
{
	...
	{
		static TensorXL<float> layer(row, col, init);//TODO: almost certainly wrong. Dont care right now
		matrix[d] = &layer;
	}
	null = false;
}
