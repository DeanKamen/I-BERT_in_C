...
#ifndef __HUBERT_TENSORMULTIPLY_XL_H__
#define __HUBERT_TENSORMULTIPLY_XL_H__
...
void matrix_multiply(TensorXL<T>* A, TensorXL<T>* B, TensorXL<T>* C) {
...
#pragma unroll
		for (int d = 0; d < BLOCK_SIZE; ++d) {
			...
			if (j >= COLSC - DOT_VEC_SIZE / BLOCK_SIZE) {
				...
				val = TensorXL<T>::get(A, rowA, colA);
			}
...
#pragma unroll
		for (int d = 0; d < DOT_VEC_SIZE; ++d) {
			running_sum += A_local_regs_stable[d] * TensorXL<T>::get(B, s * DOT_VEC_SIZE + d, j);
		}
...
#pragma unroll
		...
		if (last_s_itr && i >= 0) {
			TensorXL<T>::set(C,i,j,final_sum);
		}
		if (j == COLSC - 1) {
			j = 0;
			if (last_s_itr) {
